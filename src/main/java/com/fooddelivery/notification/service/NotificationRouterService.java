...
public class NotificationRouterService {
    ...
    public NotificationRouterService(RateLimitingService rateLimitingService, NotificationTemplateRepository templateRepository, UserPreferenceRepository userPreferenceRepository, UserDeviceRepository userDeviceRepository, NotificationAuditLogRepository auditLogRepository, List<NotificationChannelStrategy> strategies) {
        ...
        this.strategyMap = strategies.stream().collect(Collectors.toMap(NotificationChannelStrategy::getSupportedChannel, Function.identity()));
    }
    ...
    private boolean isChannelEnabled(UserPreference prefs, ChannelType channel) {
        switch (channel) {
        case SMS: 
            return prefs.getSmsEnabled();
        case PUSH: 
            return prefs.getPushEnabled();
        case EMAIL: 
            return prefs.getEmailEnabled();
        case WHATSAPP: 
            return prefs.getWhatsappEnabled();
        default: 
            return false;
        }
    }
